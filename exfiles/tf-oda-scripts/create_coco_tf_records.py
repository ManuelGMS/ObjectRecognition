...
def _create_tf_record_from_coco_annotations(annotations_file, image_dir,
  ...
  with tf.gfile.GFile(annotations_file, 'r') as fid:
    ...
    # CHANGED
    if total_num_annotations_skipped > 0:
      logging.warning('Finished writing, skipped %d annotations.', total_num_annotations_skipped)
    # CHANGED
    if keypoint_annotations_file and total_num_keypoint_annotations_skipped > 0:
      logging.warning('Finished writing, skipped %d keypoint annotations.', total_num_keypoint_annotations_skipped)
    # CHANGED
    if densepose_annotations_file and total_num_densepose_annotations_skipped > 0:
      logging.warning('Finished writing, skipped %d DensePose annotations.', total_num_densepose_annotations_skipped)
...
def main(_):
  ...
  train_output_path = os.path.join(FLAGS.output_dir, 'training.records')
  val_output_path = os.path.join(FLAGS.output_dir, 'validation.records')
  # CHANGED
  # testdev_output_path = os.path.join(FLAGS.output_dir, 'testing.records')
