...
def train_loop(
    ...
    checkpoint_max_to_keep=1000, # CHANGED
...
def eval_continuously(
  ...
  # Ultimo checkpoint generado durante el entrenamiento y su ruta.
  pathToLastCkpt = None
  
  # Este bucle obtendra el checkpoint generado mas recientemente.
  for latest_checkpoint in tf.train.checkpoints_iterator(checkpoint_dir, timeout=10, min_interval_secs=20):
    # Se obtiene el ultimo checkpoint generado durante el entrenamiento y su ruta.
    pathToLastCkpt = latest_checkpoint

  # Ruta al ultimo fichero ckpt y nombre de este.
  pathToFile, fileName = split(pathToLastCkpt)
  
  # Numero del ultimo fichero ckpt.
  fileNumber = int(fileName.split('-')[1])

  # Lista con las rutas absolutas a los ficheros ckpt.
  ckptFiles = []
  
  # Generacion de las rutas absolutas a los ficheros ckpt.
  for i in range(1, fileNumber + 1):
    ckptFiles.append(join(pathToFile, f'ckpt-{i}'))
  
  # Recorre en orden todos los ficheros de checkpoint generados durante el entrenamiento.
  for latest_checkpoint in ckptFiles:
